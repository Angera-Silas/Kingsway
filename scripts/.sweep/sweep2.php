...
foreach ($lines as $line) {
    ...
    curl_setopt_array($ch,[CURLOPT_RETURNTRANSFER=>true,CURLOPT_SSL_VERIFYPEER=>false,CURLOPT_SSL_VERIFYHOST=>false,CURLOPT_TIMEOUT=>10,CURLOPT_CONNECTTIMEOUT=>5,CURLOPT_HTTPHEADER=>$headers]);
...
$pending=$tests;
$inFlight=[];
$fh=fopen(__DIR__.'/progressive.txt','w');
$addNext=function() use (&$pending,&$inFlight,&$mh){ if(count($pending)){ $item=array_shift($pending); $inFlight[]=$item; curl_multi_add_handle($mh,$item[0]); } };
for($i=0;$i<$CONC;$i++) $addNext();
do {
    do { $status=curl_multi_exec($mh,$active); } while ($status===CURLM_CALL_MULTI_PERFORM);
    while($info=curl_multi_info_read($mh)){
        $ch=$info['handle'];
        $code=curl_getinfo($ch,CURLINFO_RESPONSE_CODE);
        foreach($inFlight as $k=>$item){ if($item[0]===$ch){ fwrite($fh, "$code ".$item[1]." ".$item[2]."\n"); unset($inFlight[$k]); curl_multi_remove_handle($mh,$ch); curl_close($ch); $addNext(); break; } }
    }
