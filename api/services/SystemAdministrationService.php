...
final class SystemAdministrationService
{
    ...
    private function rows(string $sql,array $params=[]): array { try{$s=$this->db->prepare($sql);$s->execute($params);return $s->fetchAll(PDO::FETCH_ASSOC);}catch(Throwable $e){\App\API\Services\Logger::legacyError('[SystemAdministrationService] '.$e->getMessage());return [];} }
