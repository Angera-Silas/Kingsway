...
class AttendancePermissionService
{
    ...
    public function getPermissions($id, $data, $segments, BaseController $controller) {
        ...
        try {
            ...
            $sql = "SELECT sp.*, CONCAT(s.first_name, ' ', s.last_name) as student_name, s.admission_no, c.name as class_name, cs.stream_name, st.name as student_type, st.code as student_type_code, spt.name as permission_type_name, spt.code as permission_type_code, spt.applies_to, COALESCE(CONCAT(approver_staff.first_name, ' ', approver_staff.last_name), CONCAT(approver_user.first_name, ' ', approver_user.last_name), approver_user.username) as approved_by_name FROM student_permissions sp JOIN students s ON sp.student_id = s.id LEFT JOIN class_streams cs ON cs.id = s.stream_id LEFT JOIN classes c ON c.id = cs.class_id LEFT JOIN student_types st ON st.id = s.student_type_id JOIN student_permission_types spt ON sp.permission_type_id = spt.id LEFT JOIN users approver_user ON sp.approved_by = approver_user.id LEFT JOIN staff approver_staff ON approver_staff.user_id = approver_user.id WHERE 1=1 {$streamScope['sql']}";
            ...
            if ($search !== '') { $sql .= " AND (CONCAT_WS(' ', s.first_name, s.middle_name, s.last_name) LIKE ? OR s.admission_no LIKE ? OR sp.reason LIKE ? OR spt.name LIKE ?)"; $searchTerm = '%' . $search . '%'; $params[] = $searchTerm; $params[] = $searchTerm; $params[] = $searchTerm; $params[] = $searchTerm; }
