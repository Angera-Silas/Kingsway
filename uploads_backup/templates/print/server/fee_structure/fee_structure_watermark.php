...
// Resolve local upload paths to a data URI so Dompdf can render the
// watermark consistently in development and production.
if ($sLogo !== '' && !preg_match('/^(data:|https?:\\/\\/)/i', $sLogo)) {
    if (is_file($sLogo)) {
        $candidates[] = $sLogo;
    }
    $candidates[] = dirname(__DIR__, 5) . '/' . ltrim($sLogo, '/\\');
