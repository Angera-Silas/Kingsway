...
window.API = {
  ...
  commerce: {
    ...
    stockIntakeOptions: async () => apiCall("/commerce/stock-intake-options", "GET"),
    receiveStock: async (data) => apiCall("/commerce/stock-receipts", "POST", data),
    identifyExistingStock: async () => apiCall("/commerce/stock-identify-existing", "POST", {}),
    stockUnits: async (params = {}) => apiCall("/commerce/stock-units", "GET", null, params),
    lookupStockUnit: async (code, scannerReference = "") => apiCall("/commerce/stock-unit", "GET", null, { code, scanner_reference: scannerReference }),
    stockUnitEvents: async (id) => apiCall(`/commerce/stock-unit-events/${id}`, "GET"),
    stockLabels: async (unitIds) => apiCall("/commerce/stock-labels", "POST", { unit_ids: unitIds }),
    scanOrderUnit: async (id, data) => apiCall(`/commerce/order-unit-scan/${id}`, "POST", data),
    orderPacking: async (id) => apiCall(`/commerce/order-packing/${id}`, "GET"),
    dispatchOrder: async (id, data = {}) => apiCall(`/commerce/order-dispatch/${id}`, "POST", data),
    discounts: async () => apiCall("/commerce/discounts", "GET"),
    createDiscount: async (data) => apiCall("/commerce/discounts", "POST", data),
    decideDiscount: async (id, data) => apiCall(`/commerce/discount-decision/${id}`, "PUT", data),
  },
