(() => {
    const controller = DashboardBaseController.create({
        controllerName: 'ClassTeacherDashboardController',
        rootId: 'classTeacherDashboard',
        refreshButtonId: 'classTeacherDashboardRefresh',
        stateId: 'classTeacherDashboardState',
        scopeId: 'classTeacherDashboardScope',
        lastUpdatedId: 'classTeacherDashboardLastUpdated',

        async apiMethod({ period } = {}) {
            try {
                const response = await window.API.dashboard.getClassTeacherFull({ period });
        ...
        tableDefinitions: [
            {
                bodyId: 'ctScheduleBody', path: 'tables.today_schedule', emptyText: 'No lessons scheduled today.', columns: [
                    { key: 'time' }, { key: 'subject_name' }, { key: 'room' },
                    { key: 'status', render: (v, r, c) => c.badge(v, { scheduled: 'primary', completed: 'success', cancelled: 'danger' }) }
                ]
            },
            {
                bodyId: 'ctRosterBody', path: 'tables.student_roster', emptyText: 'No students assigned.', columns: [
                    { key: 'admission_no' }, { value: r => r.student_name || [r.first_name, r.last_name].filter(Boolean).join(' ') }, { key: 'gender' },
                    { key: 'status', render: (v, r, c) => c.badge(v, { active: 'success', inactive: 'secondary', suspended: 'danger' }) }
                ]
            }
        ]
    });

    window.ClassTeacherDashboardController = controller;
    window.classTeacherDashboardController = controller;
    DashboardBaseController.boot(controller, 'ClassTeacherDashboardController');
})();
