    ...
    const controller = DashboardBaseController.create({
        ...
        async apiMethod({ period } = {}) {
            try {
                ...
                return {
                    meta: { scope_label: 'Boarding', period },
                    cards: {
                    ...
                    charts: {
                        occupancy: {
                            ...
                            datasets: [
                                { label: 'Occupied', data: occupancyRows.map(row => Number(row.occupied || 0)), borderWidth: 2 },
                                { label: 'Available', data: occupancyRows.map(row => Number(row.available || 0)), borderWidth: 2 }
                            ]
                        },
                        roll_call: {
                            labels: ['Present', 'Absent', 'Permission', 'Sick bay'],
                            data: [rollTotals.present, rollTotals.absent, rollTotals.permission, rollTotals.sickBay]
                        }
                    },
                    tables: {
                ...
                return {
                    ...
                    cards: { active_boarders: 0, occupancy_rate: 0, absent_or_unknown: 0, urgent_notes: 0 },
                    charts: { occupancy: { labels: [], datasets: [] }, roll_call: { labels: [], data: [] } },
                    tables: { roll_calls: [], exeats: [] }
                };
            }
        },
        ...
        tableDefinitions: [
            {
                bodyId: 'brdRollCallsBody', path: 'tables.roll_calls', emptyText: 'No roll-call summary for today.', columns: [
                    { key: 'dormitory_name' }, { key: 'session_name' },
                    { key: 'present_count', format: 'number' }, { key: 'absent_count', format: 'number' },
                    { key: 'attendance_percentage', format: 'percent' }
                ]
            },
            {
                bodyId: 'brdNotesBody', path: 'tables.exeats', emptyText: 'No current permissions or exeats.', columns: [
                    { key: 'student_name' },
                    { value: (row) => row.permission_type_name || row.type || 'Permission' },
                    { value: (row) => [row.start_date, row.end_date].filter(Boolean).join(' – ') },
                    { key: 'status', render: (value, row, instance) => instance.badge(value, { pending: 'warning', approved: 'success', rejected: 'danger', completed: 'secondary', cancelled: 'secondary' }) }
                ]
            }
        ]
    });
