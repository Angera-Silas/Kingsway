...
const gradeEntryCtrl = (() => {
    ...
    function toast(msg, type = 'info') {
        const el = document.getElementById('gradesToast');
        if (!el) {
            const toast = document.createElement('div');
            toast.id = 'gradesToast';
    ...
    async function loadExistingGrades(examId) {
        try {
    ...
    function renderGradesTable() {
        ...
        tbody.innerHTML = state.students.map((student, index) => {
            const existingGrade = state.grades.find(g => g.student_id === student.id);
            const marks = existingGrade?.marks || '';
            const grade = existingGrade?.grade || '';
            const remarks = existingGrade?.remarks || '';
            ...
            return `
                <tr data-student-id="${student.id}">
                    ...
                    <td>
                        <input type="number" 
                               class="form-control grades-input" 
                               min="0" 
                               max="${maxMarks}" 
                               step="0.5"
                               value="${marks}"
                               ...
                               onchange="gradeEntryCtrl.calculateGrade(this)">
                    </td>
                    <td>
    ...
    function updateStats() {
        ...
        const graded = state.students.filter(student => {
            const input = document.querySelector(`.grades-input[data-student-id="${student.id}"]`);
    ...
    async function saveAllGrades() {
        ...
        const gradingData = [];
        const inputs = document.querySelectorAll('.grades-input');
        ...
        if (!gradingData.length) {
            toast('No grades to save', 'warning');
            return;
        }
    ...
    async function exportGrades() {
        ...
        if (window.PrintManager) {
            ...
            window.PrintManager.exportToCSV({
                filename: `exam_grades_${state.currentExam.name}_${new Date().toISOString().slice(0,10)}.csv`,
                columns: columns,
                rows: rows
            });
        } else {
