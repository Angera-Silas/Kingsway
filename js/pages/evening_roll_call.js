/**
 * evening_roll_call.js — Evening Roll Call Controller (standalone page).
 * Night roll call attendance per dormitory.
 */
const eveningRollCallController = {
  SESSION_CODE: 'NIGHT_ROLL_CALL',
  state: { rows: [] },
  ...
  async load() {
    const body = document.getElementById('ercBody');
    body.innerHTML = '<tr><td colspan="9" class="text-center py-3"><div class="spinner-border spinner-border-sm"></div></td></tr>';
    try {
      const date = document.getElementById('ercDate')?.value || new Date().toISOString().slice(0, 10);
      const r = await this.API('GET', '/boarding/roll-call', null, { date });
      this.state.rows = this.norm(r).filter(x => x.session_code === this.SESSION_CODE || x.session_name?.includes('Night'));
      this.render();
  ...
  render() {
    const body = document.getElementById('ercBody');
    if (!this.state.rows.length) { body.innerHTML = '<tr><td colspan="9" class="text-center py-4 text-muted">No evening roll call data for the selected date.</td></tr>'; return; }
  ...
  setupEventListeners() {
    document.getElementById('ercDate')?.addEventListener('change', () => this.load());
  },
...
window.eveningRollCallController = eveningRollCallController;

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', () => eveningRollCallController.init().catch(() => {}));
} else {
  eveningRollCallController.init().catch(() => {});
}
