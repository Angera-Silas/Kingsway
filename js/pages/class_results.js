/**
 * Class Results Controller - Class Teacher Specific Results Viewing
 * Role: Class Teacher (7)
 * Shows only results for the teacher's assigned classes
 * Integrates with AcademicContext for academic year awareness
 */

const classResultsController = (() => {
    const state = {
        results: [],
        classes: [],
        subjects: [],
    ...
    async function loadResults() {
        try {
            ...
            const response = await apiCall('academic/performance-overview', 'GET', {
                ...
                class_teacher_only: true // Only show results for this teacher's classes
            });

            // Map performance-overview rows to the per-student result shape this page renders.
            state.results = (response.data?.rows || []).map(r => ({
                student_id: r.student_id,
                student_name: r.full_name || `${r.first_name || ''} ${r.last_name || ''}`.trim(),
                admission_no: r.admission_no,
                class_name: r.class_name,
                subject_name: r.subject_name || subjectId || '—',
                marks: r.average_score ?? 0
            }));
            renderResults();
            updateStats();
        } catch (error) {
            ...
            tbody.innerHTML = `
                <tr>
                    <td colspan="8" class="text-center text-muted py-4">
                        No results found for your classes
                    </td>
                </tr>
            `;
            return;
    ...
    async function exportResults() {
        ...
        if (window.PrintManager) {
            ...
            window.PrintManager.exportToCSV({
                filename: `class_results_${new Date().toISOString().slice(0,10)}.csv`,
                columns: columns,
                rows: rows
            });
        } else {
