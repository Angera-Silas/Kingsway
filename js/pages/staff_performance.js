...
const StaffPerformanceController = {
    ...
    async init() {
        ...
        if (!AuthContext.canView('staff')) {
            showNotification('You do not have permission to view staff performance', 'error');
            return;
        }
        ...
        if (exportBtn) {
            exportBtn.addEventListener("click", () => {
                if (AuthContext.canExport('staff')) {
                    ...
                    showNotification('You do not have permission to export reports', 'error');
                }
            });
        }
    ...
    async loadStaffList() {
        try {
            ...
            this.staff = this.extractStaffList(normalized.data);
    ...
    async loadCurrentAcademicYear() {
        try {
            ...
            const normalized = AppState.normalizeResponse(response);
            const year = normalized.data || null;
    ...
    async generateReport() {
        ...
        if (!staffId) {
            showNotification("Select a staff member to generate a report.", "warning");
            return;
        }
    ...
    async loadReviewHistory(staffId) {
        try {
    ...
    async loadKpiSummary(staffId) {
        try {
            const params = this.currentYearId ? { academic_year_id: this.currentYearId } : {};
            const response = await window.API.staff.getAcademicKPISummary(staffId, params);
            const normalized = AppState.normalizeResponse(response);
            const payload = normalized.data || {};
    ...
    renderPerformanceTable() {
        ...
        if (!this.reviews.length) {
            tbody.innerHTML = '<tr><td colspan="8" class="text-center text-muted py-3">No performance data found</td></tr>';
            return;
        }
        ...
        const studentResults = this.getKpiPercent(["student", "performance", "results"]);
    ...
    renderAppraisalHistory() {
        ...
        if (!this.reviews.length) {
            tbody.innerHTML = '<tr><td colspan="6" class="text-center text-muted py-3">No appraisal history found</td></tr>';
            return;
        }
    ...
    async showReport(reviewId) {
        try {
            const response = await window.API.staff.generatePerformanceReport(reviewId);
            const normalized = AppState.normalizeResponse(response);
            showNotification(normalized.message || "Performance report generated.", "info");
        } catch (error) {
    ...
    exportReport() {
        ...
        const headers = ['Date', 'Period', 'Reviewer', 'Overall Score', 'Grade', 'Comments'];
        ...
        a.download = 'staff_performance.csv';
        a.click();
    }
};
