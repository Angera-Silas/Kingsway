...
const enterMarksCtrl = (() => {
    ...
    async function loadAssessments() {
        try {
            ...
            const response = await apiCall('academic/formative-assessments', 'GET', {
                ...
                term_id: termId,
                class_id: classId,
                teacher_only: true // Only show assessments for this teacher's classes
            });
            ...
            if (select) {
                ...
                state.assessments.forEach(assessment => {
                    ...
                    option.textContent = `${assessment.name} (${assessment.class_name || 'All Classes'})`;
                    select.appendChild(option);
                });
            }
        } catch (error) {
    ...
    async function loadStudentsForAssessment(assessmentId) {
        try {
            ...
            // Load students for the assessment's class
                ...
                <tr data-student-id="${student.id}">
                    ...
                    <td>
                        <input type="number" 
                               ...
                               onchange="enterMarksCtrl.calculateGrade(this)">
                    </td>
                    <td>
    ...
    async function exportMarks() {
        ...
        if (window.PrintManager) {
            ...
            window.PrintManager.exportToCSV({
                filename: `marks_${state.currentAssessment.name}_${new Date().toISOString().slice(0,10)}.csv`,
                columns: columns,
                rows: rows
            });
        } else {
    ...
    function bindEvents() {
        ...
        document.getElementById('classFilter')?.addEventListener('change', loadAssessments);
