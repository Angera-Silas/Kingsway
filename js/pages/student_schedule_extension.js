const studentScheduleExtensionController = {
    async init() {
        ...
        modal.addEventListener('shown.bs.modal', async () => {
            ...
            if (!studentId) return;
            this.ensureScheduleTab();
            await this.loadStudentSchedule(studentId);
        });
    },
    ...
    renderScheduleRow(entry) {
        ...
        const timeRange = `${this.formatTime(entry.start_time)} - ${this.formatTime(entry.end_time)}`;
    ...
    renderHolidayItem(holiday) {
        ...
        const when = this.formatDate(holiday.date || holiday.holiday_date || holiday.start_date);
    ...
    renderScheduleContent(schedules, holidays) {
        const scheduleRows = schedules.length
            ? schedules.map(row => this.renderScheduleRow(row)).join('')
        ...
        const holidaysList = holidays.length
          ...
          ${holidays.map(h => this.renderHolidayItem(h)).join('')}
        </ul>`
    ...
    async loadStudentSchedule(studentId) {
        ...
        try {
            ...
            schedulePane.innerHTML = this.renderScheduleContent(schedules, holidays);
        } catch (error) {
...
window.loadStudentSchedule = (studentId) => studentScheduleExtensionController.loadStudentSchedule(studentId);

if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => studentScheduleExtensionController.init().catch(() => {}));
} else {
