/**
 * Subject Schemes of Work Controller
 * Page: subject_schemes_of_work.php
 * Subject Teacher-specific view of schemes for their assigned subjects
 * Integrates with AcademicContext for academic year awareness
 */
const SubjectSchemesOfWorkController = {
  state: {
    schemes: [],
    subjects: [],
  ...
  async loadSchemes() {
    try {
      ...
      this.state.schemes = await window.API.academic.getSubjectSchemes(params) || [];
        ...
        <tr>
          ...
          <td>
            <div class="btn-group btn-group-sm">
              <button class="btn btn-outline-primary" onclick="SubjectSchemesOfWorkController.viewScheme(${scheme.id})" title="View Scheme">
                <i class="bi bi-eye"></i>
              </button>
              <button class="btn btn-outline-secondary" onclick="SubjectSchemesOfWorkController.editScheme(${scheme.id})" title="Edit Scheme">
                <i class="bi bi-pencil"></i>
              </button>
              <button class="btn btn-outline-success" onclick="SubjectSchemesOfWorkController.submitForApproval(${scheme.id})" title="Submit for Approval">
  ...
  async submitForApproval(id) {
    if (!(await window.confirmAction('Submit for Approval', 'Submit this scheme for approval?'))) return;
  ...
  exportSchemes() {
    ...
    if (window.PrintManager) {
      window.PrintManager.exportToCSV({
        headers,
        rows
      }, 'subject_schemes_of_work');
    } else {
      // Fallback
      let csv = headers.join(',') + '\n' + 
        rows.map(r => r.map(v => '"' + (v || '') + '"').join(',')).join('\n');
      
      KingswayFileLifecycle.exportText(csv, 'subject_schemes_of_work.csv', 'text/csv');
    }
  },
