-- Generate obligations only for the current term and terms remaining in the
-- active academic year. Fee schedule amounts are already per-term amounts;
-- never use an annual total as the amount for every term.
...
BEGIN
    ...
    IF v_enrollment_id IS NULL OR v_student_type_id IS NULL THEN
        SET p_obligations_created = 0;
    ELSE
        -- p_term_id accepts an academic_year_terms id. NULL means the active
        -- term, with a date-based fallback for installations that do not keep
        -- the term status flag current.
